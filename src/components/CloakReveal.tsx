  ...
  return (
    <div className="panel overflow-hidden">
      <div className="flex items-center justify-between border-b border-line px-5 py-3">
        ...
        >
          <span className={showMoney ? 'text-muted-fg' : 'text-accent'}>
            {safeLabel}
          </span>
          <span className="relative h-5 w-10 rounded-full border border-accent/40 bg-ink-800">
            <span
              className="absolute top-0.5 h-3.5 w-3.5 rounded-full bg-accent transition-transform duration-300"
              style={{
                transform: showMoney ? 'translateX(22px)' : 'translateX(3px)',
              }}
            />
          </span>
          <span className={showMoney ? 'text-accent' : 'text-muted-fg'}>
            {moneyLabel}
          </span>
        </button>
      </div>
